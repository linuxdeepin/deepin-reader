...
Q_DECLARE_FLAGS(RenderPaperFlags, RenderPaperFlag)
...
public:
    RenderParam(int resolutionX = 72, int resolutionY = 72, qreal devicePixelRatio = 1.0,
                ...
                RenderPaperFlags flags = 0) : d(new SharedData)
    {
    ...
    RenderPaperFlags flags() const { return d->flags; }
    void setFlags(RenderPaperFlags flags) { d->flags = flags; }
    void setFlag(RenderPaperFlag flag, bool enabled = true)
    {
        if(enabled)
        {
            d->flags |= flag;
