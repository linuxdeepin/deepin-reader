...
#include <QMutex>
static QMutex mutexrenderimage;
...
bool XpsPage::loadPageWords(void *words)
{
...
//                qCWarning(OkularXpsDebug) << "Unhandled element in Text Extraction start: " << xml.name().toString();
            ...
            if (xml.name() == QStringLiteral("Canvas")) {
                ...
                for (int i = 0; i < text.length(); i++) {
                    ...
                    stWord sword;
                    sword.s = text.mid(i, 1);
                    QRectF rect( origin.x() + lastWidth,
                                 origin.y() - metrics.height(),
                                 width - lastWidth,
                                 metrics.height());
                    sword.rect = matrix.mapRect( rect );
//                    rect->transform( matrix );
//                    textPage->append( text.mid(i, 1), rect );
                    qDebug() << "sword s:" << sword.s << " rect:" << sword.rect << " pageSize:" << m_pageSize;
                    ((QList<stWord> *)words)->append(sword);
