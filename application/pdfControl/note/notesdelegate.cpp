...
void NotesDelegate::paint(QPainter * painter, const QStyleOptionViewItem & option, const QModelIndex & index) const
{
    if (index.isValid()){
        ...
        if(!pixmap.isNull()){
            ...
            const ImagePageInfo_t &tImagePageInfo = index.data(ImageinfoType_e::IMAGE_CONTENT_TEXT).value<ImagePageInfo_t>();
            QString contentText = tImagePageInfo.strcontents;
