...
{
    qCDebug(appLog) << "BrowserMenu::initActions type:" << type << "index:" << index << "fileType:" << sheet->fileType();
    m_type = type;
    m_pColorWidgetAction = nullptr;
    if (type == DOC_MENU_ANNO_ICON) {
        qCDebug(appLog) << "BrowserMenu::initActions() - Processing DOC_MENU_ANNO_ICON";
        createAction(tr("Copy"), "CopyAnnoText");
        this->addSeparator();

        if (sheet->fileType() == Dr::FileType::PDF || sheet->fileType() == Dr::FileType::DOCX) {
            qCDebug(appLog) << "BrowserMenu::initActions() - Adding PDF/DOCX annotation actions";
        ...
        if (sheet->hasBookMark(index)) {
            ...
            qCDebug(appLog) << "BrowserMenu::initActions() - Adding add bookmark action";
            createAction(tr("Add bookmark"), "AddBookmark");
        }
    } else if (type == DOC_MENU_ANNO_HIGHLIGHT || type == DOC_MENU_SELECT_TEXT) {
        qCDebug(appLog) << "BrowserMenu::initActions() - Processing DOC_MENU_ANNO_HIGHLIGHT or DOC_MENU_SELECT_TEXT";
        if (type == DOC_MENU_ANNO_HIGHLIGHT) {
            qCDebug(appLog) << "BrowserMenu::initActions() - Creating copy annotation action";
            QAction *copyAnnoAction = createAction(tr("Copy"), "CopyAnnoText");
            if (copytext.isEmpty()) {
                qCDebug(appLog) << "BrowserMenu::initActions() - Disabling copy action due to empty text";
                copyAnnoAction->setDisabled(true);
            }
        } else {
            qCDebug(appLog) << "BrowserMenu::initActions() - Creating copy action";
        ...
        if (sheet->fileType() == Dr::FileType::PDF || sheet->fileType() == Dr::FileType::DOCX) {
            ...
            if (type == DOC_MENU_ANNO_HIGHLIGHT) {
                qCDebug(appLog) << "BrowserMenu::initActions() - Enabling remove highlight for annotation highlight";
        ...
        if (sheet->isFullScreen()) {
            qCDebug(appLog) << "BrowserMenu::initActions() - Adding exit fullscreen action";
            createAction(tr("Exit fullscreen"), "ExitFullscreen");
        } else {
            qCDebug(appLog) << "BrowserMenu::initActions() - Adding fullscreen action";
        ...
        if (sheet->currentIndex() == 0) {
            qCDebug(appLog) << "BrowserMenu::initActions() - Disabling first/previous page actions (at first page)";
        ...
        if (sheet->currentIndex() == sheet->pageCount() - 1) {
            qCDebug(appLog) << "BrowserMenu::initActions() - Disabling next/last page actions (at last page)";
        ...
        if (sheet->fileType() == Dr::FileType::PDF || sheet->fileType() == Dr::FileType::DOCX) {
            qCDebug(appLog) << "BrowserMenu::initActions() - Adding annotation action for PDF/DOCX";
            createAction(tr("Add annotation"), "AddAnnotationIcon");
        }
    ...
    qCDebug(appLog) << "BrowserMenu::initActions() - Init actions completed";
}

void BrowserMenu::hideEvent(QHideEvent *event)
{
    // qCDebug(appLog) << "BrowserMenu hidden";
    emit sigMenuHide();

    DMenu::hideEvent(event);
    // qCDebug(appLog) << "BrowserMenu::hideEvent() - Hide event completed";
}

QAction *BrowserMenu::createAction(const QString &displayname, const QString &objectname)
{
    // qCDebug(appLog) << "BrowserMenu::createAction:" << objectname;
    QAction *action = new  QAction(displayname, this);
    action->setObjectName(objectname);
    connect(action, SIGNAL(triggered()), this, SLOT(onItemClicked()));
    this->addAction(action);
    // qCDebug(appLog) << "BrowserMenu::createAction() - Action created and added to menu";
    return action;
}
...
void BrowserMenu::onItemClicked()
{
    ...
    qCInfo(appLog) << "BrowserMenu item clicked:" << actionName;
    emit signalMenuItemClicked(actionName);
    qCDebug(appLog) << "BrowserMenu::onItemClicked() - Emitted signalMenuItemClicked with action:" << actionName;
}

void BrowserMenu::onSetHighLight()
{
    // qCDebug(appLog) << "BrowserMenu::onSetHighLight type:" << m_type;
    if (m_type == DOC_MENU_SELECT_TEXT) {
        // qCDebug(appLog) << "BrowserMenu::onSetHighLight() - Emitting AddTextHighlight signal";
        emit signalMenuItemClicked("AddTextHighlight");
    } else if (m_type == DOC_MENU_ANNO_HIGHLIGHT) {
        // qCDebug(appLog) << "BrowserMenu::onSetHighLight() - Emitting ChangeAnnotationColor signal";
        emit signalMenuItemClicked("ChangeAnnotationColor");
    }
    this->close();
    // qCDebug(appLog) << "BrowserMenu::onSetHighLight() - Set highlight handler completed";
}
