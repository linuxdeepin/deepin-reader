...
public:
    ...
    QVariant data(const QModelIndex &index, int role) const
    {
        ...
        case 2:
            return font.isEmbedded() ? PDFiumDocument::tr("Yes") : PDFiumDocument::tr("No");
        case 3:
            return font.isSubset() ? PDFiumDocument::tr("Yes") : PDFiumDocument::tr("No");
        case 4:
            return font.file();
        default:
            return QVariant();
...
public:
    ...
    bool object(const PDFiumPage *page, TextBoxList &textBoxes) const
    {
    ...
    void insert(const PDFiumPage *page, const TextBoxList &textBoxes)
    {
        QMutexLocker mutexLocker(&m_mutex);
    ...
    void remove(const PDFiumPage *page)
    {
        QMutexLocker mutexLocker(&m_mutex);
...
private:
    ...
    QCache< const PDFiumPage *, TextBoxList > m_cache;
...
PDFiumAnnotation::PDFiumAnnotation(QMutex *mutex, Poppler::Annotation *annotation) : Annotation(),
    m_mutex(mutex),
...
QList<QRectF> PDFiumAnnotation::boundary() const
{
    LOCK_ANNOTATION
...
QString PDFiumAnnotation::contents() const
{
    LOCK_ANNOTATION
...
Poppler::Annotation *PDFiumAnnotation::ownAnnotation()
{
    return m_annotation;
}

bool PDFiumAnnotation::updateAnnotation(const QString contains, const QColor color)
{
    LOCK_ANNOTATION
...
bool PDFiumAnnotation::setUniqueName(QString uniqueName) const
{
    LOCK_ANNOTATION
...
QString PDFiumAnnotation::uniqueName() const
{
    if (nullptr == m_annotation)
        return QString();
...
PDFiumPage::PDFiumPage(QMutex *mutex, Poppler::Page *page) :
    m_mutex(mutex),
    m_page(page)
{
}
...
QImage PDFiumPage::render(Dr::Rotation rotation, const double scaleFactor, const QRect &boundingRect) const
{
    return render(72 * scaleFactor, 72 * scaleFactor, rotation, boundingRect);
}

QImage PDFiumPage::render(int width, int height, Qt::AspectRatioMode) const
{
...
QImage PDFiumPage::render(qreal horizontalResolution, qreal verticalResolution, Dr::Rotation rotation, QRect boundingRect) const
{
    LOCK_LONGTIMEOPERATION
...
QImage PDFiumPage::thumbnail() const
{
    LOCK_PAGE
...
QString PDFiumPage::label() const
{
    LOCK_PAGE
...
QList< Link * > PDFiumPage::links() const
{
    LOCK_PAGE
...
QString PDFiumPage::text(const QRectF &rect) const
{
    LOCK_PAGE
...
QString PDFiumPage::cachedText(const QRectF &rect) const
{
...
QList<Word> PDFiumPage::words(Dr::Rotation rotation)
{
...
QList< QRectF > PDFiumPage::search(const QString &text, bool matchCase, bool wholeWords) const
{
    LOCK_PAGE
...
QList< Annotation * > PDFiumPage::annotations() const
{
    LOCK_PAGE
    ...
    foreach (Poppler::Annotation *annotation, m_page->annotations()) {
        if (annotation->subType() == Poppler::Annotation::AText || annotation->subType() == Poppler::Annotation::AHighlight || annotation->subType() == Poppler::Annotation::AFileAttachment) {
            annotations.append(new PDFiumAnnotation(m_mutex, annotation));
            continue;
        }
...
bool PDFiumPage::canAddAndRemoveAnnotations() const
{
...
Annotation *PDFiumPage::addHighlightAnnotation(const QList<QRectF> &boundarys, const QString &text, const QColor &color)
{
    LOCK_PAGE
    ...
    return new PDFiumAnnotation(m_mutex, annotation);
...
bool PDFiumPage::removeAnnotation(deepin_reader::Annotation *annotation)
{
    LOCK_PAGE

#ifdef HAS_POPPLER_20

    deepin_reader::PDFiumAnnotation *PDFiumAnnotation = static_cast< deepin_reader::PDFiumAnnotation * >(annotation);

    if (PDFiumAnnotation == nullptr)
        return false;

    m_page->removeAnnotation(PDFiumAnnotation->m_annotation);

    PDFiumAnnotation->m_annotation = nullptr;
    delete PDFiumAnnotation;
    PDFiumAnnotation = nullptr;
...
QList< FormField * > PDFiumPage::formFields() const
{
    LOCK_PAGE
...
bool PDFiumPage::updateAnnotation(Annotation *annotation, const QString &text, const QColor &color)
{
...
bool PDFiumPage::mouseClickIconAnnot(QPointF &clickPoint)
{
...
Annotation *PDFiumPage::addIconAnnotation(const QRectF rect, const QString text)
{
    if (nullptr == m_page)
        return nullptr;
...
Annotation *PDFiumPage::moveIconAnnotation(Annotation *annot, const QRectF rect)
{
...
PDFiumDocument::PDFiumDocument(Poppler::Document *document) :
    m_mutex(),
    m_document(document)
{
}
...
int PDFiumDocument::numberOfPages() const
{
    LOCK_DOCUMENT
...
Page *PDFiumDocument::page(int index) const
{
    LOCK_DOCUMENT

    if (Poppler::Page *page = m_document->page(index)) {
        return new PDFiumPage(&m_mutex, page);
    }

    return nullptr;
}

bool PDFiumDocument::isLocked() const
{
    LOCK_DOCUMENT
...
bool PDFiumDocument::unlock(const QString &password)
{
    LOCK_DOCUMENT
...
QStringList PDFiumDocument::saveFilter() const
{
    return QStringList() << "Portable document format (*.pdf)";
}

bool PDFiumDocument::canSave() const
{
    return true;
}

bool PDFiumDocument::save(const QString &filePath, bool withChanges) const
{
    LOCK_DOCUMENT
...
bool PDFiumDocument::canBePrintedUsingCUPS() const
{
    return true;
}

void PDFiumDocument::setPaperColor(const QColor &paperColor)
{
    LOCK_DOCUMENT
...
Outline PDFiumDocument::outline() const
{
    Outline outline;
...
Properties PDFiumDocument::properties() const
{
...
QAbstractItemModel *PDFiumDocument::fonts() const
{
    LOCK_DOCUMENT
...
bool PDFiumDocument::wantsContinuousMode() const
{
    LOCK_DOCUMENT
...
bool PDFiumDocument::wantsSinglePageMode() const
{
    LOCK_DOCUMENT
...
bool PDFiumDocument::wantsTwoPagesMode() const
{
    LOCK_DOCUMENT
...
bool PDFiumDocument::wantsTwoPagesWithCoverPageMode() const
{
    LOCK_DOCUMENT
...
bool PDFiumDocument::wantsRightToLeftMode() const
{
...
PDFiumDocument *PDFiumDocument::loadDocument(const QString &filePath, const QString &password)
{
...
//        document->setRenderHint(Poppler::Document::ThinLineShape, true);
        ...
        return new deepin_reader::PDFiumDocument(document);
    }
